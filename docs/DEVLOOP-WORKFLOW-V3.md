# DevLoop 开发中枢 — 系统工作模式 (V3.1 终态)

> **文档定位**: 本文是 DevLoop 系统的"工作模式"权威描述,补充 `DEVLOG-V2.3-MASTER.md` 的"架构/数据/API"视角,聚焦"谁在什么时机做什么、产物流向何处"
>
...
DevLoop 系统的**最终形态**是一个"自进化、自调度、自修复"的项目指挥中枢。任何一个新方案**通过任何通道**进入系统后,**无需人为干预**就能完成"分析→拆解→建表盘→派任务→审查→修复→复盘→再审查"的**自进化闭环**。

### 1.1 一句话目标

> **方案从 4 入口(后台/飞书/企微/微信) 进入 → DevLoop 自主完成"方案解析→建项目表盘→智能审查(代际进化)→派修复任务→复盘迭代"全流程 → 每次审查都比上次更精准**
...
| `frontend/DevLoops.vue` | 284 | 370 | +Agent 实时状态面板 (5 Agent 卡片 + 心跳 + 当前任务) |
...
> 当前 DevLoop 离"4 入口接入 + 代际进化智能审查 + 自修复"项目指挥中枢, 还差 **36 项 + 20h P0 任务**。
